...
export function SkillPageDocument({ skill, file, tabKey, tabs, projectId }: { skill: string; file: string; tabKey: string; tabs: TabsApi; projectId?: string }) {
  ...
  useEffect(() => {
    const listener = (event: Event) => {
      const detail = (event as CustomEvent<{ skill?: string; actor?: string }>).detail;
  ...
  const srcDoc = useMemo(() => (content !== null && isHtml ? withBridge(content, readStorage(storageKey)) : ""), [content, isHtml, storageKey]);
...
function withBridge(html: string, storage: Record<string, string>) {
...
window.mbox={embedded:true,send:function(text){parent.postMessage({type:"mbox:send",text:String(text)},"*");},close:function(){parent.postMessage({type:"mbox:close"},"*");}};
})();</script>`;
