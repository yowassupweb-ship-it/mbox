...
const OPENAI_ACCENT = "#10a37f";
const GEMINI_ACCENT = "#1a73e8";

// Canonical brand marks (single-path, 24×24) sourced from simple-icons.
...
function brandGlyph(path: string): ReactNode {
...
export function agentIdentity(name: string): AgentIdentity {
  ...
  if (key.includes("claude") || key.includes("anthropic")) {
    return { key: "claude", label: raw, accent: CLAUDE_ACCENT, glyph: brandGlyph(CLAUDE_PATH) };
  }
  if (key.includes("codex") || key.includes("chatgpt") || key.includes("gpt") || key.includes("openai")) {
    return { key: "openai", label: raw, accent: OPENAI_ACCENT, glyph: brandGlyph(OPENAI_PATH) };
  }
...
export function AgentAvatar({ name, status = "idle", live = false, size = 34 }: { name: string; status?: string; live?: boolean; size?: number }) {
  ...
  return (
    <span
      className={`agent-avatar ${identity.key} ${stateClass}`}
