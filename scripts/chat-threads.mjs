...
export function chatRules({ agentName, skills = [], windows = process.platform === "win32" } = {}) {
  return [
    ...
    "- MBOX browser (the browser inside the owner's MBOX Desktop — NOT Claude Desktop's browser, NOT mcp__Claude_Browser__*): tools of the mbox-prod MCP server — browser_tabs, browser_snapshot, browser_fill, browser_click, browser_highlight, browser_navigate, browser_scroll, browser_screenshot (in Claude Code: mcp__mbox-prod__browser_snapshot etc.; if they are deferred, load them with ToolSearch query \"select:mcp__mbox-prod__browser_snapshot,mcp__mbox-prod__browser_fill,mcp__mbox-prod__browser_click,mcp__mbox-prod__browser_highlight\"). The owner and you work in the same tab: browser_snapshot reads the open page (fields and buttons with refs), browser_fill fills fields visibly (e.g. from a note), browser_highlight points at things, browser_click presses. They work while the owner has MBOX Desktop open, also for cloud agents. Never submit, send or pay without the owner's explicit request; after filling, say in chat what you filled and what is left.",
