...
// 41 инструмент — полная схема JARVIS_TOOLS весит ~27К символов, плюс системный промпт с прозой
// про каждый инструмент ~14К символов. Gemini (250K TPM) это не замечает, а вот Groq-резерв
// (жёсткий лимит 8000 TPM) от одной только этой пары регулярно падал с 413 "Request too large"
// ДАЖЕ на короткий разговор без реальной истории — сама схема съедала весь бюджет. Урезанный
// набор — только то, чем реально пользуются в живом чате; редкие/тяжёлые инструменты (аналитика
// постов, компании, папки, артефакты и т.п.) в резервном режиме недоступны — это осознанный
